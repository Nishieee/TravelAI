...
def is_travel_related_gpt(query):
    ...
    try:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an AI assistant specialized in determining whether a query is travel-related. A query can be considered travel-related if it mentions: \n- Keywords like 'places,' 'itinerary,' 'trip,' 'travel,' or 'journey.' \n - Names of countries, cities, landmarks, tourist attractions, or geographic regions.\n- Activities or topics associated with travel, such as 'visa,' 'flights,' 'hotels,' 'restaurants,' 'local cuisine,' or 'transportation.'\n\nExamples of travel-related queries include:\n- 'Top places to visit in Istanbul.'\n- 'Paris in 2 days itinerary.'\n- 'How to travel in Iran.'\n- 'Indian food stores in Pakistan.'\n\nIf a query doesn't meet the above criteria, return a message indicating the query is not travel-related. Do not process irrelevant or malformed queries, including ones with technical errors or unrelated content (e.g., 'insufficient_quota' errors or unrelated logs). Provide feedback politely if a query doesn't fit travel-related criteria."
            },
            {
                "role": "user",
                "content": f"Is the following query related to travel? Respond with 'Yes' or 'No' only.\n\nQuery: {query}"
            }
        ]
...
If the user asks for an itinerary for a specific duration (e.g., "3 days in Paris"), structure your response by days and make sure to keep the title highlighted always bold it to heading 3:
- Day 1: Morning, afternoon, and evening activities.
- Day 2: Repeat with a similar structure.
- Add extra details for each activity (e.g., timings, location, and why it's worth visiting) make sure to bold the keywords such that the main places come out as highlited.
            """
        },
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",  # Specify GPT model
            messages=messages,
